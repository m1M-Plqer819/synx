--[[
    redz UI lib source code
    credit: redz999 
    modified by: player.81

    note:
    no need to save it haha stupid kids (Not convenient to name)
    it's free to use but It is still in development
--]]
  ...
  function Window:MakeTab(paste, Configs)
    ...
    function Tab:AddDropdown(Configs)
      ...
      local function AddOption(OptionName)
          ...
          local function CallbackFunc()
              if Flag and typeof(Flag) == "string" then
                  SaveFile(Flag, Multi and MultSelected or Selected[1])
              end
              task.spawn(Callback, Multi and MultSelected or Selected[1])
              table.foreach(MultCallback, function(_, func)
                  if type(func) == "function" then
                      task.spawn(func, Multi and MultSelected or Selected[1])
                  end
              end)
          end
          ...
          local function Active()
              if Multi then
                  if table.find(MultSelected, OptionName) then
                      table.remove(MultSelected, table.find(MultSelected, OptionName))
                  else
                      table.insert(MultSelected, OptionName)
                  end
                  ActiveLabel.Text = table.concat(MultSelected, ", ")
                  for _, Val in pairs(Options) do
                      if table.find(MultSelected, Val.Name) then
                          CreateTween({ Val.Label, "TextTransparency", 0, 0.35 })
                          CreateTween({ Val.Frame, "BackgroundTransparency", 0, 0.35 })
                          CreateTween({ Val.Frame, "Size", UDim2.fromOffset(4, 14), 0.35 })
                      else
    ...
    function Tab:Addprofile(Configs)
      ...
      local dmn = Configs[1] or Configs.Desc or Configs.Description or ""
